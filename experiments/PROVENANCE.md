...
- **Required reads:**
  ...
  - `experiments/notebooks/data/workshop_set_manifest.json` — numbers for §Results (AUROC, CI95, Brandes anchor).
  - `shared/bib/references.bib` — available citations. Every methodology cite must carry a `doi` field (validator enforces).
  - `experiments/EXPERIMENT_LOG.md` — §Discussion scars (2026-05-13 LLR sign-and-formula fix; 2026-05-12 MAX_LEN off-by-2, crashed on BRCA1 L1854P — the demo pair).
- **Required figure:** `experiments/analysis/figures/resolution_panels.pdf` (already on the graphicspath).
- **Validator:**       `validate_paper.py --real-paper` — bites on hallucinated DOI, unresolved figure, stale headline AUROC.
- **Fallback:**        `paper/main_fallback.{tex,pdf}` — pre-generated against this contract, validator-clear. If the live demo stalls, narrate against the fallback PDF.
- **Repo-state contract:** the paper is derivative of repo state. If the manifest's `llr_auroc` changes, the prose must change. If `compute_llr` changes signature, §Methods must change. If a new scar lands in `EXPERIMENT_LOG.md`, §Discussion gets a paragraph. The generator regenerates the paper; the validator enforces it; this entry binds them.
